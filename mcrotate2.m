...
% mc2frontal
% 
% Original code from the Motion Capture Toolbox, Copyright  2008,
% University of Jyvaskyla, Finland
%
% Modified by Kristian Nymoen 2020. Allowing input of marker number as
% rotation point.
% 
...
if length(point) == 1
    %allows to input marker number instead of an actual point
    mx = d.data(:,3*point-2); my = d.data(:,3*point-1); mz = d.data(:,3*point); %KN changed 2020-01-15
else
    mx(1:d.nFrames) = point(1); my(1:d.nFrames) = point(2); mz(1:d.nFrames) = point(3);
end
...
for k=1:size(x,2) % per marker
    ...
    if ~isempty(ind) %% added 250511 PT
        ...
        w = quatrot([x(ind,k)-mx(ind),y(ind,k)-my(ind),z(ind,k)-mz(ind)], q);
        x(ind,k) = w(:,1);
        y(ind,k) = w(:,2);
        z(ind,k) = w(:,3);
    end
